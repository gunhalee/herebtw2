...
욕설·혐오·선정성·우회 표현과 대규모 moderation 확장 계획은
[콘텐츠 안전·유해 표현 탐지 구현 설계 및 실행 계획](./content-moderation-implementation-plan.md)을
기준으로 한다.
...
콘텐츠 차단·격리 문구, 최대 12시간 수동 검수, 사용자 재검토 경로 없음, 후보자 답변 제외,
이건하 단독 운영 정책은 상단에 연결한 콘텐츠 안전 계획에서 확정되었다. 다음 항목만 구현 전
담당자가 결정해야 하지만 P0~P2 진행을 막지는 않는다.

1. 사용자가 자신의 신고를 취소할 수 있게 할지
2. 본인 포스트 신고를 허용할지
3. 후보가 답변할 수 있는 지역 범위를 제한할지
4. 투표 일시·장소 관련 콘텐츠의 공식 출처
5. 이메일 알림 확인 절차와 보존 기간
6. 동일 구호 캠페인을 서비스에서 어떻게 표시할지
