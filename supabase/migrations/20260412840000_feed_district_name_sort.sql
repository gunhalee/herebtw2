-- Feed: area_code_prefix prefix 매칭 방식을 선거구명 직접 매칭으로 대체.
--
-- area_code_prefix(시군구 5자리)는 같은 구 내 모든 구시군의원이 매칭되어 너무 넓음.
-- candidates.local_council_district / metro_council_district 이름 비교로 정확한 선거구 매칭.
--
-- 정렬 우선순위:
--   1. viewer의 구시군의원 선거구 후보 답변 (agree_count desc)
--   2. viewer의 시도의원 선거구 후보 답변 (agree_count desc)
--   3. 그 외 answered 글 (distance asc)
--   4. 일반 글 (distance asc, created_at desc)

-- area_code_prefix 컬럼 제거 (사용되지 않음)
alter table public.candidates
  drop column if exists area_code_prefix;

-- 기존 viewer_dong_code 시그니처 함수 제거
...
as $$
  ...
  with_replies as (
    select
      ...
      c.local_council_district as reply_candidate_local_district,
      c.metro_council_district as reply_candidate_metro_district,
  ...
  select
    ...
    case
      when wr.has_viewer_identity = false then false
      else true
    end as can_report,
    wr.reply_status,
  ...
  left join agree_counts
    on agree_counts.post_id = wr.id
  left join viewer_agrees
    on viewer_agrees.post_id = wr.id
  order by
    -- 1순위: viewer의 구시군의원 선거구 후보 답변 (agree_count desc)
    case
      when wr.reply_status = 'replied'
        and viewer_local_council_district is not null
        and wr.reply_candidate_council_type in ('구의회', '시의회', '군의회')
        and wr.reply_candidate_local_district = viewer_local_council_district
      then 0
      -- 2순위: viewer의 시도의원 선거구 후보 답변 (agree_count desc)
      when wr.reply_status = 'replied'
        and viewer_metro_council_district is not null
        and wr.reply_candidate_council_type = '시도의회'
        and wr.reply_candidate_metro_district = viewer_metro_council_district
      then 1
      else 2
    end asc,
    -- 지역구 매칭 답변 내에서 agree_count 내림차순
    case
      when wr.reply_status = 'replied'
        and (
          (
            viewer_local_council_district is not null
            and wr.reply_candidate_council_type in ('구의회', '시의회', '군의회')
            and wr.reply_candidate_local_district = viewer_local_council_district
          ) or (
            viewer_metro_council_district is not null
            and wr.reply_candidate_council_type = '시도의회'
            and wr.reply_candidate_metro_district = viewer_metro_council_district
          )
        )
      then coalesce(agree_counts.agree_count, 0)
      else null
    end desc nulls last,
    -- 3순위: 그 외 answered 글
