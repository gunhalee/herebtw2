-- Feed: viewer_dong_code 파라미터 추가 + 지역구 후보 답변 최우선 정렬.
--
-- 정렬 우선순위:
--   1. viewer의 동 코드가 후보의 area_code_prefix로 시작하는 answered 글 (agree_count desc)
--   2. 그 외 answered 글 (distance asc)
--   3. 일반 글 (distance asc, created_at desc)
--
-- 성능 영향:
--   - LIMIT 이전 구조 변경 없음 (extra join 없음)
--   - with_replies CTE에 컬럼 1개 추가 (기존 join)
--   - 최종 ORDER BY에 starts_with() 문자열 비교 추가 (무시 가능)
...
create or replace function public.list_posts_feed(
  ...
  viewer_dong_code text default null
)
returns table (
  id uuid,
  public_uuid uuid,
...
as $$
  ...
  with_replies as (
    select
      ...
      c.area_code_prefix as reply_candidate_area_prefix,
  ...
  order by
    -- 1순위: viewer 지역구 후보가 답변한 글
    case
      when wr.reply_status = 'replied'
        and wr.reply_candidate_area_prefix is not null
        and viewer_dong_code is not null
        and starts_with(viewer_dong_code, wr.reply_candidate_area_prefix)
      then 0
      else 1
    end asc,
    -- 지역구 답변 글 내에서 agree_count 내림차순
    case
      when wr.reply_status = 'replied'
        and wr.reply_candidate_area_prefix is not null
        and viewer_dong_code is not null
