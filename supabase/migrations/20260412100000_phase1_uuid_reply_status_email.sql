-- Phase 1: Add uuid (external identifier), reply_status, notification_email to posts

alter table public.posts
  add column if not exists public_uuid uuid unique not null default gen_random_uuid();

alter table public.posts
  add column if not exists reply_status text not null default 'delivered'
    constraint posts_reply_status_check check (reply_status in ('delivered', 'replied'));

alter table public.posts
  add column if not exists notification_email varchar(255);

create index if not exists idx_posts_public_uuid
  on public.posts (public_uuid)
  where status = 'active';

-- Postgres rejects CREATE OR REPLACE when the OUT/return row type changes; drop first.
...
-- Update the list_posts_feed function to include public_uuid and reply_status
...
-- Function to find a post by public_uuid (for the /v/[uuid] page)
