...
- 서버가 확인한 익명 기기 기준 `15초에 1회, 1시간에 10회 작성 가능` 정책을 적용한다.
...
- 기본 정책은 `15초에 1회, 1시간에 10회 작성 가능`이다.
...
- `15초 1회·1시간 10회 작성 제한`은 DB의 원자적 budget RPC와 애플리케이션 로직으로 처리한다.
...
- `15초 1회·1시간 10회` rate limit와 정규화된 완전 동일 문장 차단을 적용한다.
