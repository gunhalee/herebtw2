...
const FROM_EMAIL =
  process.env.REPLY_NOTIFICATION_FROM?.trim() ||
  "여기 근데 한마디 할게요 <onboarding@resend.dev>";
const BASE_URL =
  process.env.REPLY_NOTIFICATION_BASE_URL?.trim().replace(/\/$/, "") ||
  "https://herebtw.vercel.app";
...
export async function sendReplyNotification(
  ...
  const cardImageUrl = `${BASE_URL}/api/card/${input.publicUuid}?type=voter`;
  const cardDownloadUrl = `${cardImageUrl}&download=1`;
  const voiceUrl = `${BASE_URL}/v/${input.publicUuid}`;
  ...
  try {
    ...
    let cardAttachment:
      ...
      | undefined = input.cardAttachment
        ? {
            filename: cardFileName,
            content: input.cardAttachment,
    ...
    if (input.cardAttachment === undefined) {
      try {
    ...
    const cardHtml = input.cardAttachment === null
      ? `<div style="text-align: center; margin: 0 0 24px;"><a href="${voiceUrl}" style="display: block; width: 100%; max-width: 240px; margin: 0 auto; background: #111827; color: #ffffff; text-align: center; padding: 12px 22px; border-radius: 999px; text-decoration: none; font-size: 14px; font-weight: 700; line-height: 1.3; box-sizing: border-box;">답변 확인하기</a></div>`
      : `<div style="text-align: center; margin: 0 0 24px;"><img src="${cardInlineSrc}" alt="답변이 반영된 포토카드" width="320" style="display: block; width: 100%; max-width: 320px; height: auto; margin: 0 auto 14px; border-radius: 14px; border: 1px solid #e5e7eb;" /><a href="${cardDownloadUrl}" style="display: block; width: 100%; max-width: 240px; margin: 0 auto; background: #111827; color: #ffffff; text-align: center; padding: 12px 22px; border-radius: 999px; text-decoration: none; font-size: 14px; font-weight: 700; line-height: 1.3; box-sizing: border-box;">포토카드 다운로드</a></div>`;
    ...
    if (error) {
      console.error("[email] Resend rejected reply notification:", error.name);
